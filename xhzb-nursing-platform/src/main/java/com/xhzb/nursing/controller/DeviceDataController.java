...
public class DeviceDataController extends BaseController
{
    ...
    @Log(title = "设备数据表", businessType = BusinessType.EXPORT)
    @PostMapping("/export")
    @Operation(summary = "导出设备数据表列表")
    public void export(HttpServletResponse response, DeviceData deviceData)
    {
        ...
        util.exportExcel(response, list, "设备数据表数据");
    }
    ...
    @GetMapping(value = "/{id}")
    @Operation(summary = "获取设备数据表详细信息")
    public AjaxResult getInfo(@Schema(name = "设备数据表ID", requiredMode = Schema.RequiredMode.REQUIRED)
	...
	@DeleteMapping("/{ids}")
    @Operation(summary = "删除设备数据表")
    public AjaxResult remove(@Schema(name = "设备数据表ID", requiredMode = Schema.RequiredMode.REQUIRED) @PathVariable Long[] ids)
    {
